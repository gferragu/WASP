...
module retrieve_gf
   ...
   &  dxs, dys, nx_p, ny_p, dip_seg, stk_seg, cniu, t_latest
...
!      read(9,*) no, sta_name(ir), lat_s(ir), lon_s(ir), io_mod(ir), component(ir), weig, nos(ir)
      ...
      io_chan = io_chan+1
      ll_g = io_chan+ll_in
      if (channel2 .eq. 'Z') filename = trim(sta_name1(ir))//'1'
      if (channel2 .eq. 'N') filename = trim(sta_name1(ir))//'2'
      if (channel2 .eq. 'E') filename = trim(sta_name1(ir))//'3'
...
!      read(9,*) no, sta_name(ir), lat_s(ir), lon_s(ir), io_mod(ir), component(ir), weig, nos(ir)
      ...
      io_chan = io_chan+1
      ll_g = io_chan+ll_in
      if (channel2 .eq. 'Z') filename = trim(sta_name2(ir))//'.cgps.1'
      if (channel2 .eq. 'N') filename = trim(sta_name2(ir))//'.cgps.2'
      if (channel2 .eq. 'E') filename = trim(sta_name2(ir))//'.cgps.3'
      filename = trim(filename)
...
!
         ...
         do iys = 1, nys_sub(i_s)
            do ixs = 1, nxs_sub(i_s)
               ...
               do iyp = 1, ny_p
                  do ixp = 1, nx_p
                     kpxy = kpxy+1
                     time = point_sources(4, ixp, iyp, ixs, iys, i_s)/v_ref+delay_seg(i_s)
                     time = time + tdel(kpxy, kxy, i_s, ir)
                     do i = 1, 2*jfmax
                        W = -twopi*(i-1)*df*time
                        z = cmplx(0.0, w)
                        z = cexp(z)
               ...
               do i = 1, 2*jfmax
                  if (disp_or_vel(ir) .eq. 0) then
                     z = cmplx(1.0, 0.0)
                  elseif (disp_or_vel(ir) .eq. 1) then
                     w = twopi*(i-1)*df
                     z = cmplx(0.0, w)
                  end if
   ...
   &  npp, ix, iy, nxy, j, ll_g, ll, io_chan, i, k, io_mod(max_stations), &
   ...
   real fint(wave_pts), f1, f2, f3, f4, const_c, tsub(max_psources), h, omega, &
...
!                     sour_sub(i) = sour_sub(i)+cmplx(cos(a), sin(a))
   ...
   subroutine get_surface_waves_gf2(ll_in, ll_out)
   ...
   real*8 :: dip, theta, dis, az, baz, rad_c, coef_v(2, 3), coef_r(2, 5)
   real*8 :: cr0(wave_pts), ci0(wave_pts)
   real :: cr(wave_pts), ci(wave_pts)
   real fint(wave_pts), f1, f2, f3, f4, const_c, tsub(max_psources), h, omega, &
   &  dist_min, dist_max, depth_sub, dep_min, dep_max, lat_p, lon_p, niu, &
   &  lat_sta, lon_sta, df_bank, tlen_bank, &
   &  time, a, block, lat_s(max_stations), lon_s(max_stations), dt, rake, &
   ...
   complex :: kahan_y, kahan_t, kahan_c
   complex :: surf_gf_dip(wave_pts), surf_gf_stk(wave_pts)
   complex sour_sub(wave_pts), green_s(wave_pts2, 10), www, wss, z0
...
!
   ...
   do ir = 1, ir_max
      ...
      do i_ch = 1, 3
         ...
         elseif (i_ch .eq. 1 .and. io_up(ir) .eq. 1) then
            rad_c = 0.0
            filename = trim(sta_name(ir))//'.bhz.surf'
            filename = trim(filename)
            open(35, file=filename, form='unformatted', &
                & access='direct', recl=974*8)
         end if
         if (i_ch .eq. 2.and.io_ns(ir).ne.1) then
            cycle
         elseif (i_ch .eq. 2 .and. io_ns(ir) .eq. 1) then 
            rad_c = ang_ns(ir)
            filename = trim(sta_name(ir))//'.sh.surf'
            filename = trim(filename)
            open(35, file=filename, form='unformatted', &
                & access='direct', recl=974*8)
         end if
...
!
               do ixs = 1, nxs_sub(i_s)
                  ...
                  cr0(:) = 0.d0
                  ci(:) = 0.0
                  read(35, rec=2*ll-1) cr0(:974)
                  cr(:) = real(cr0(:))
                  call fft(cr, ci, lnpt, -1.0)
                  surf_gf_dip(:) = cmplx(cr(:), ci(:)) 
                  cr(:) = 0.0
                  ci(:) = 0.0
                  read(35, rec=2*ll) cr0(:974)
                  cr(:) = real(cr0(:))
                  call fft(cr, ci, lnpt, -1.0)
                  surf_gf_stk(:) = cmplx(cr(:), ci(:)) 
                  nxy = (iys-1)*nxs_sub(i_s)+ixs
                  lat_p = point_sources(1, int(nx_p/2) + 1, int(ny_p/2) + 1, ixs, iys, i_s)
                  lon_p = point_sources(2, int(nx_p/2) + 1, int(ny_p/2) + 1, ixs, iys, i_s)
                  niu = cniu(nxy, i_s)
...
!                     sour_sub(i) = sour_sub(i)+cmplx(cos(a), sin(a))
   ...
   return
   end subroutine get_surface_waves_gf2
